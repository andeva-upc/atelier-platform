...
public class ProductsController {
    ...
    public ProductsController(ProductCommandService productCommandService) { this.productCommandService = productCommandService; }
    @PostMapping
    public ResponseEntity<ProductResource> createProduct(@RequestBody CreateProductResource resource) {
        CreateProductCommand command = new CreateProductCommand(new BranchId(resource.branchId()), ProductCategory.valueOf(resource.category()), new ProductName(resource.name()), new Sku(resource.sku()), resource.description(), new Money(resource.salePrice()), new InventoryQuantity(resource.minimumStock()));
        Optional<Product> product = productCommandService.handle(command);
        return product.map(p -> new ResponseEntity<>(ProductResourceFromAggregateAssembler.toResourceFromAggregate(p), HttpStatus.CREATED)).orElseGet(() -> ResponseEntity.badRequest().build());
    }
}
