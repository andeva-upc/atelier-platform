...
public class ProductEntityAssembler {
    public static ProductJpaEntity toEntity(Product product) {
        ...
        var batchEntities = product.getBatches().stream().map(b -> {
            ...
            be.setInitialQuantity(b.getAvailableQuantity().value()); 
    ...
    public static Product toAggregate(ProductJpaEntity entity) {
        ...
        Product p = new Product(entity.getId(), new BranchId(entity.getBranchId()), ProductCategory.valueOf(entity.getCategory()), new ProductName(entity.getName()), new Sku(entity.getSku()));
        if (entity.getBatches() != null) {
            for (ProductBatchJpaEntity be : entity.getBatches()) {
                p.addBatch(new ProductBatch(be.getBatchId(), new InventoryQuantity(be.getInitialQuantity()), new Money(be.getAcquisitionCost())));
            }
        }
        return p;
    }
